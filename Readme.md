...
Let $G\_1$ and $G\_2$ be finite groups with isomorphic subgroups identified through an amalgamated subgroup (A). We consider the amalgamated free product
...
!\[Image](/docs/D12\_example.png "Cayley graph of D12")
* the cyclic group $C_4$ of order 4:
!\[Image](/docs/C4\_example.png "Cayley graph of C4")
* an amalgamated subgroup identifying the reflection $b\in D_{12}$ with $c^2\in C_4$.

```python
from stallings\\\\\\\_amalgams import get\\\\\\\_red\\\\\\\_precover
from stallings\\\\\\\_amalgams.groups import create\\\\\\\_D2n, create\\\\\\\_Cn
from stallings\\\\\\\_amalgams.visualization import visualize

# create\\\\\\\_D2n(n) constructs the dihedral group with 2n elements.
D12 = create\\\\\\\_D2n(6)
...
P = get\\\\\\\_red\\\\\\\_precover(D12, C4, A, H)
visualize(P)
```

The resulting graph is the reduced precover associated with the subgroup generated by `H`.

!\[Image](/docs/D12\_C4\_example.png "Precover corresponding to H")

Images are obtained with Tikz and Latex with datatotex() function in Stallings\\\_amalgams/stallings\\\_amalgams/visualization.py.
